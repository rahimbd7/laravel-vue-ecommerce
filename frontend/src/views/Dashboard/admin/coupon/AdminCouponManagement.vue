  ...
  <div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Coupon Management</h1>
          <p class="text-sm text-gray-600">Manage platform-wide coupons and discounts</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <Button 
            ...
            icon="pi pi-plus" 
            severity="primary"
            @click="router.push('/dashboard/admin/coupons/create')"
          />
          <Button 
            icon="pi pi-download" 
            label="Export" 
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
      ...
      >
        ...
        <Column field="is_public" header="Public" style="min-width: 80px" class="hidden sm:table-cell">
          <template #body="{ data }">
            <Tag 
          ...
          <template #body="{ data }">
            <div class="flex items-center gap-1 justify-end">
              ...
              <Button 
                ...
                size="small"
                severity="info"
                @click="router.push(`/dashboard/admin/coupons/${data.id}/edit`)"
                tooltip="Edit"
                class="!w-7 !h-7"
              />
              <Button 
...
const stats = computed(() => {
  ...
  // Count active coupons (is_active = true AND not expired AND not exhausted)
  ...
  // Count expired coupons (expires_at is in the past)
...
const fetchCoupons = async () => {
  loading.value = true
  try {
    const response = await adminApi.getCoupons(filters)
...
const fetchAllCoupons = async () => {
  try {
    const response = await adminApi.getCoupons({ 
      per_page: 9999, // Get all coupons for stats
      status: '', 
      search: '' 
    })
    const data = response.data?.data || response.data || []
    allCoupons.value = Array.isArray(data) ? data : [data]
    console.log('📊 All coupons loaded for stats:', allCoupons.value.length)
  } catch (error) {
...
const toggleStatus = async (coupon: any) => {
  try {
    await adminApi.toggleCouponStatus(coupon.id)
    toast.add({
      severity: 'success',
      summary: 'Success',
...
const viewCoupon = (coupon: any) => {
  router.push(`/dashboard/admin/coupons/${coupon.id}`)
}

const exportCoupons = async () => {
  try {
    const response = await adminApi.exportCoupons(filters)
