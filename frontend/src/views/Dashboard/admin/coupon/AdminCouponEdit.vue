    ...
    <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          />
          <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Coupon</h1>
      ...
      <form @submit.prevent="updateCoupon" class="space-y-4 sm:space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div>
            ...
            <small v-else class="text-gray-400 text-xs">Must be unique (e.g., SUMMER20, WELCOME10)</small>
          </div>
...
const form = ref({
  ...
  eligible_customer_types: [] as string[],
  is_public: true,
  is_active: true,
...
const fetchCoupon = async () => {
  loading.value = true
  try {
    const response = await adminApi.getCoupon(route.params.id as string)
...
const updateCoupon = async () => {
  ...
  try {
    ...
    const response = await adminApi.updateCoupon(route.params.id as string, payload)
    ...
    router.push('/dashboard/admin/coupons/management')
  } catch (error: any) {
